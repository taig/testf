...
object Tests {
  val count: Test[Id, _] => Int = {
    case Test.Allocation(test, _) => count(test)
  ...
  val label: Test[Id, _] => Option[String] = {
    case Test.Allocation(test, _)   => label(test)
  ...
  def children[A]: Test[Id, A] => List[Test[Id, A]] = {
    case Test.Allocation(test, _) => children(test)
  ...
  val throwable: Test[Id, _] => Option[Throwable] = {
    case Test.Allocation(test, _) => throwable(test)
